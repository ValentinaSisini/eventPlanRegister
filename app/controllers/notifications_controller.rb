class NotificationsController < ApplicationController
  ...
  before_action :authenticate_user!, :set_notification, only: %i[ show edit update destroy ]
  load_and_authorize_resource

  # GET /notifications or /notifications.json
  def index
    @notifications = Notification.where(user_id: current_user.id).includes(:event, :user)
  end
  ...
  private
    ...
    def notification_params
      params.require(:notification).permit(:user_id, :event_id, :message, :datetime)
    end
end
